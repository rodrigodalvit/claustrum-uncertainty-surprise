...
for neuron = 1:size(pathsResp,1)
    ...
    a=randi([1000 100000],size(eventTimes,1),1);
    eventTimes = eventTimes + a;
    numTrials = length(eventTimes);


    baseEventTimes = allAppearingAsteroidTimes;
    b = randi([1000 100000],size(eventTimes,1),1);
    baseEventTimes = baseEventTimes + b;
    baseEventTimes = allAppearingAsteroidTimes;
...
baseIndexFR = find(edges>=-1000 & edges <-500);
...
binsToCheck = find(edges>=0 & edges <4000);
